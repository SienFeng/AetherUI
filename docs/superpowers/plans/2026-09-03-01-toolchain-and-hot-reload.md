...
//
// 断言不能把两边都 unmarshal 成 map[string]any 再比较：那样做会同时把
// got 与 want 都归一化（键重排、空白压掉），一个「整份模板 map[string]any
// 往返」的坏实现（丢失字节保真，正是本设计要避开的反模式）也能让两边看起来
// 相等，从而蒙混过关——这条测试曾经就是这么写的，红-绿验证时发现它对该
// 回归完全没有检出力（Task 6 fix round 1 的教训，参见 task-6-report.md）。
// 改为直接在输出文本里找未受影响键的原始字节。
//
// 两个值的字面量刻意写成键序非字母序，且不含任何多余空白：
...
// 差异守不住任何东西。真正只有 RawMessage 透传才保得住、map[string]any
// 往返保不住的，是嵌套对象内部的键序——map[string]any 在重新序列化时会把
// 每一层的 map key 按字母序重排，RawMessage 因为把子树当不透明字节块搬运，
// 键序原样不动。所以这里选的键序本身就不是字母序（handshake < statsUserUplink，
