...
**空 remark 不参与第 1 级匹配，直接退到 port。** `DBInbound.remark` 的默认值就是空串，入站表单也不强制填，所以「备注为空的入站」是本项目的合法状态、且是偷懒管理员的默认状态——不是理论输入。`ref.Remark` 为 `""` 时若仍去按 remark 查表，会在本机**恰好也有一个**备注为空的入站时静默命中它：`missing` 为空、覆盖集不塌缩，规则以文件里的 `enable`（通常是 `true`）落库，绑在一个与源机器毫无关系的用户身上，且因为「命中了」而**连「认不全就禁用」这道兜底都不会触发**。这与下面 §5.3.1 的域名组重名歧义同构，但更危险：域名组歧义好歹会走「整条拒绝」的分支，空 remark 误配则连自己「有歧义」都不知道。
...
**`domainGroupRef` 为空字符串同样不参与匹配，在查表之前就整条拒绝。** 空业务键当键用，会在本机恰好存在一个备注为空的域名组时静默命中它——这不是假想输入：`domainGroupRef` 为空本来就对应源机器上一个备注为空的域名组（导出侧忠实保留了 `g.Remark`），两台机器各自存在一个未命名域名组是完全可达的组合。命中之后产生的是一条指向错误域名组、看起来完全正常的规则，与上面「两个同名组」的歧义是同一类错误，处置也一致：拒绝，不猜。
