...
**回退到旧版本二进制**：`domain_group_id` 列保留不删。契约收敛成一句话——**只有升级前就存在、且升级后未被编辑过的规则，回退后仍按原样生效；其余一律被旧代码整条丢弃**（`buildRule` 记 Warning，分流范围缩小而非放大，安全侧正确）。

三条写入路径各自的产出：

| 路径 | `domain_group_id` | 回退后旧代码的行为 |
|---|---|---|
| 迁移（升级时回填一次） | 保持原值不动 | 原样生效 |
| `RoutingRuleService.Add` | 恒为 0（只写 `DomainGroupIds`），**单组多组一视同仁** | 整条丢弃 |
| `RoutingRuleService.Update` | **显式置 0** | 整条丢弃 |

`Update` 置 0 是刻意的，不是遗漏，另外两种写法都更糟：

- **保留旧值**：一条从组 3 改到组 9 的规则会留下 `domain_group_ids=[9]` 而 `domain_group_id=3`，回退后按管理员**已经从规则里删掉的**组 3 分流。既不是丢弃也不是范围缩小，而是一条谁都没要求过的规则，且 xray 返回 `Configuration OK`、面板首页显示 `running`，没有任何一层会报错。
- **取首个组的 id**：多组规则回退后静默退化成只按其中一个组分流，同样无人要求。

让文档描述一个二选一的简单行为，远好过描述一个随编辑历史分岔的三岔行为。这一点应写进 CLAUDE.md。
