...
的逐字节副本，转发路径一模一样；这一条对有连接池语义的协议有一处已知例外，见 §6.3 末尾）；新连接建立的额外开销在微秒量级，而建立一条 TCP
...
```
该入站最近 24 小时（小时桶），**只把 bytes(d) > 0 的域名计入分子和分母**：
  avgBytesPerConn = Σ[bytes(d)>0] (up+down) / Σ[bytes(d)>0] count   （分母为 0 时取 0）
  权重(d) = bytes(d) > 0 ? bytes(d) : count(d) × avgBytesPerConn
排序键：权重 desc, count desc, domain asc
```

**分子分母都只收「真的量到过字节」的域名**，不是对全部域名求和。`bytes == 0` 的行
既可能是「没被计量」也可能是「计量了但确实没流量」，从聚合结果里分不出来；把它们
的连接数放进分母，就是拿没量过的连接去稀释量过的连接的均值。新装机器上池里只有极
少数域名，这一稀释能到几十倍——而这个估算存在的唯一理由就是防止池锁死在首日那批域
名上，稀释它等于让它静默失效（既不报错，也没有任何界面表征）。实现见
`web/service/meter_pool.go` 的 `meterAvgBytesPerConn`。
...
| 试用退场 | 在池且连续 `meterProbeGiveUpRounds`(3) 轮**窗口内**字节为 0 的域名退池，并设 `CooldownUntil = now + meterCooldown`(24h)。**从进池那一轮算起一共是 4 轮** | 一个只有连接数、没有流量的域名（探测器、失败重连）长期占着槽位 |

`MeterPoolJob` 频率 `@every 1h`，所以「轮」就是小时。

试用退场这一行有两处必须照实记下来的偏差，都是实现比原设计更稳的一侧：

1. **实际需要 4 轮，不是 3 轮。**第 0 轮是「进池」，那一轮 `inPool` 里还没有这个域名，
   零轮计数的循环压根不会为它赋值；计数从其后每一轮才 +1，跑满 3 需要再走 3 轮。
   多一轮观察期与这道闸门的目的一致（少误杀一个刚进池、恰好那一小时没流量的域名），
   代价只是槽位多占一小时。回归测试 `TestRecomputeRetiresZeroByteDomainAndCoolsItDown`
   把这个 4 显式写进了循环次数。
2. **判据是「窗口内字节为 0」，不是「本轮字节为 0」。**实现直接复用
   `recomputeInbound` 顶部那次 24 小时窗口的聚合（`meterPoolWindow`），不另做一次
   「只看上一小时」的查询——多一条查询就多一处会与排序口径漂移的地方。后果是一个
   20 小时前有过流量、之后一直为零的域名，`ProbeZeroRounds` 每轮都被清零，退不了场，
   白占槽位最长约 27 小时（24h 窗口 + 3 轮）。这道闸门真正要挡的那类（探测器、失败
   重连、已被管理员规则分流走的域名）字节**恒为 0**，窗口里也没有非零行，照常退场，
   所以危害有限且落在保守侧（少退场 ≠ 错误归因，只是槽位周转慢一点）。
...
type MeterDomain struct {
    ...
    // 里留下行，这个状态只能由池表自己记。
    ProbeZeroRounds int
    // CooldownUntil 之前该域名不参选，Unix 秒；0 表示不在冷却、正在池内。
    CooldownUntil int64
}
```

**「在池」的判据必须是精确的 `cooldown_until = 0`，绝不是 `<= now`。**这一列同时承担
「是不是在池」和「冷却到什么时候」两个语义，`<= now` 会把 `0 < cooldown_until <= now` 的
行——已退场、冷却已过期、等着被下一次 `Recompute` 删掉——也当成在池。那个窗口最长一小时
（清理只在 `Recompute` 顶部跑，而任何一次面板重启都会打乱 cron 相位，窗口均匀分布在
`[0, 1h)`），足以让 §6.2.4 的试用退场闸门每 24 小时被规律性击穿一次：一个刚被判「不值得
再试」的域名会在下一次 `Recompute` 之前自己回到生成配置里，既绕过冷却判定，也绕过「重新
参选要和别人比权重」这一步。§6.2.5 的冻结分支更甚——它在那条清理之前就 `return`，于是过
期冷却行会一直被当成在池，它们的 tag 从死计数器观测里被剔出去，观测值下降、冻结提前解除，
方向与冻结的目的正好相反。

`= 0` 与「未退场」严格等价，靠三条写入约束：**创建时不写 `CooldownUntil`（零值）、退场时
写未来时刻、冷却期满是删行而不是改回 0**。动这三处任何一条之前先回到这里。同一个判据有两
个消费方——`MeterPoolService.Pool`（生成期）与 `DomainStatService.inboundIsMetered`（榜单
的 `Metered` 字段），两者漂移会让界面为一个其实已经全部退场的入站显示两列恒为 0 的字节。

相应地，`Recompute` 顶部那次冷却清理的 `RowsAffected` **刻意不计进返回的 `changed`**：
这些行删除前后都不在池内，池的可见内容一个字节都没变，计进去只会让 `MeterPoolJob` 白置
一次重启标志。真正的可见变化是它们随后重新入选，那走的是进池分支，本来就会计数。
...
**「库不可用」包括「句柄在但查询失败」，不只是句柄为 nil。**`RoutingInjector.Inject` 拿到
读池错误时必须记一条 Warning 并按空池继续，绝不能 `return err`：`GetXrayConfig` 一失败，
`RestartXray` 就在拿到配置那一步返回，根本走不到 `NewProcess`；而面板重启后 xray 本来就
没在跑，`CheckXrayRunningJob` 每次触发都卡在同一处——**xray 从此永远起不来、全员断网**，
唯一的信号只有一行日志。用量库是每 10 秒被 `TrafficBucket`/`InboundIPHour`/`DomainStat`
三路写入、每小时还有一次可能扫过一年数据的清理 DELETE 的独立 SQLite 文件，`SQLITE_BUSY`
超过 go-sqlite3 默认 5s busy timeout、磁盘满、文件损坏都是现实触发条件。这是整个注入链
路上唯一一处必须 fail-open 的错误。
...
JSON 往返做深拷贝，理由就是紧接着的那次 `clone["tag"] = ...`：浅拷贝（把模板首位那个
map 直接 append 进去）会让全部克隆与数组首位的默认出站是同一个对象，于是默认出站被写上
最后一个计量 tag，而 `diffOutbounds` 要求首位逐字节不变——**每换一次池都退化成整进程
重启**。回归测试 `TestInjectAppendsMeterOutboundsAsDefaultCopies` 里那条「首位的 tag 不是
计量 tag」的断言正是抓这个的。

（不是「§6.5 的 DNS 注入会串写 `settings`」：`DNSInjector.applyFreedomStrategy` 是从
`cfg.OutboundConfigs` 的**字节**重新 `Unmarshal` 的，而计量出站那些 map 在那之前就已经
被 `Marshal` 成字节了，内存里的 map 共享在这之后一律不可能。）

追加位置在 `a-ui-block` 黑洞出站**之后**、数组最末。首位仍是默认出站（`diffOutbounds`
要求它逐字节不变），黑洞出站的位置也不受影响。

**一个已知代价：§6.0 的「吞吐完全不受影响」只对 freedom 成立。**那句话的依据是「计量出站
是默认出站的逐字节副本，转发路径一模一样」——对无连接池语义的 freedom 确实如此。但如果
管理员把模板首位出站换成带 `mux` 的 vmess/vless（也就是把「直连」定义成「走我自己的中转」），
那么计量出站就是多个独立 handler、多套独立的 mux 客户端，对上游的控制连接数被放大
一个量级（总数最大 `meterOutboundBudget` = 200，单入站最大 `meterPoolCapMax` = 60）。**刻意不改代码**：给计量出站自己造一个 freedom
会破坏「除 tag 外与默认出站逐字节相同」这条更重要的不变量——那条不变量保证被计量的流量
与其它直连流量走**完全同一条路径**，破坏它就等于让榜单量到的是另一条链路的数字。这个代价
只记录，不补偿。
...
- **深拷贝**：数组首位的默认出站不会被写上计量 tag（浅拷贝的可观测判据；污染它就会让
  `diffOutbounds` 判「必须重启」，每换一次池都退化成整进程重启）。
...
- **池的三道闸门**：最小驻留（进池 1 轮的域名不被挤出）、替换余量（权重差 1.1 倍不换、1.5 倍换）、
  试用退场（零字节域名从进池那一轮起共 4 轮后退池并冷却 24 小时，冷却期内不再入选；见 §6.2.4
  为什么是 4 不是 3）。
- **「在池」的判据是 `cooldown_until = 0`**：一行「已退场、冷却已过期、还没被下一次 `Recompute`
  删掉」的记录既不能出现在 `Pool()` 的返回里，也不能让榜单的 `Metered` 为真——判据一旦放宽成
  `<= now`，试用退场闸门每 24 小时被击穿一次，死计数器冻结也会被反向解除（见 §6.2.6）。
...
| 节点网速变慢 | 实测排除：400 条规则相对 0 条只多约 8 µs，在噪音以内；吞吐完全不受影响（§6.0）。唯一例外是默认出站带 `mux` 时上游控制连接数被放大，见 §6.3 末尾 |
| 榜单聚合拖慢面板 | 聚合在独立库、独立 job、单轮有行数上限；查询走桶不走原始日志 |
| 用量库打不开导致配置生成失败 | 池为空即不生成任何计量出站与规则，fail-open 到「没有这个功能」，不影响 xray 配置生成。**句柄为 nil 与查询失败两条路径都要 fail-open**——后者 fail-close 会让 xray 永远起不来（见 §6.2.6） |
