...
2. **判据是「窗口内字节为 0」，不是「本轮字节为 0」。**实现直接复用
   ...
   白占槽位最长约 27 小时（24h 窗口 + 4 轮）。这道闸门真正要挡的那类（探测器、失败
...
那么 K 个计量出站就是 K 个独立 handler、K 套独立的 mux 客户端，对上游的控制连接数被放大
一个量级（K 最大 `meterPoolCapMax` = 60）。**刻意不改代码**：给计量出站自己造一个 freedom
