...
**时区改变后的行为，比「交界处错位一天」严重得多。** `History` 与 `Overview`（`web/service/traffic_history.go`）是用 `bucket_start` **精确相等**去 join `buildSlots` 按**当前**时区重算出的刻度；旧桶的 `bucket_start` 不变（按当时的时区切），改时区后这两个集合通常**不相交**，不是错开一格：

- 任何整小时时区切换（UTC ↔ Asia/Shanghai ↔ America/New_York 这类常见情况）下，改动前的**日桶全部**落不到新刻度上，「1 年」这一档的历史数据整体消失（实测 0/365 匹配）。小时桶能存活，因为整小时偏移不改变 `bucket_start ≡ 0 (mod 3600)`。
- 改到或改离半小时/一刻钟时区（Asia/Kolkata、Asia/Tehran、Australia/Adelaide、Asia/Kathmandu）时，改动前的**小时桶也全部**落不上，24h / 7d / 30d 的历史数据一并消失（实测 0/720 匹配）。
- `Overview` 里排 Top N 的那个聚合查询（`sum(up+down)` group by `inbound_id`）**没有**对齐约束，所以错位的行照样把入站顶进 Top 12——管理员看到的是十二条图例、十二条平坦的 0 线，观感像图坏了，而不是时区变了。

数据在盘上完好，没有丢失；随着保留期内新数据按新对齐写入，图会逐渐自愈。刻意不做补偿——重切历史桶需要原始的秒级数据，而那份数据早已聚合掉了；任何「估算着搬」的补偿逻辑都是在用假数据覆盖真数据。正解是把读路径的精确相等换成「落在 `[slot, nextSlot)` 区间内」的判定，但那是结构性改动，本次不做。
...
写入频率：10 秒一次 × 有流量的入站数 × 2（小时桶 + 日桶）。10 个活跃入站也就 720 次 UPSERT/小时，对一个独立的、没有读竞争的 SQLite 库微不足道——**但这只是语句数，不是提交数**。GORM 的 `SkipDefaultTransaction` 默认是 `false`（本项目从没设过它），每一次 `Create` 都自带一个独立的 `BEGIN...COMMIT`；`Record`（`web/service/traffic_history.go`）把一轮里 N 个入站的 2N 次 `upsertBucket` 调用包进一个 `db.Transaction(...)`，`2N` 次提交因此变成 1 次。不包的话，SQLite 默认回滚日志下每次提交约 2 次 fsync，N=15 时就是每轮约 60 次、持续约 6 次/秒，且与主库、访问日志库共享同一块盘——这才是真正的成本来源。包成一个事务的副作用是这一轮采集变成原子的：磁盘中途出错是整轮不写，而不是写了一半。
...
| 取舍 | 说明 |
|---|---|
| 时区改变后历史数据在图上整体消失 | 见 §3.3。读路径按 `bucket_start` 精确相等 join 当前时区重算的刻度，不是错位一格：整小时切换让日桶全灭（1 年档清空），半小时/一刻钟时区连小时桶一起灭（24h/7d/30d 一并清空）。`Overview` 的 Top N 排序不受对齐约束，misaligned 的入站照样能顶进前 12，观感是十二条平的 0 线，像故障而非时区变了。数据不丢，保留期内随新数据写入自愈。重切历史需要早已聚合掉的原始数据，估算补偿等于用假数据覆盖真数据 |
...
| 夏令时回拨那天，两个真实的小时会对齐到同一个桶 | `AlignHour` 按本地时间取整点，回拨当天 01:00~02:00 会走两遍。两遍都对齐到同一个 `bucket_start`，那个桶装两小时的量，相邻刻度显示 0。已实测：`America/New_York` 2026-11-01，01:30 EDT 与 01:30 EST 都对齐到 `1793509200`。面板默认时区 `Asia/Shanghai` 无夏令时，只影响把时区设成有 DST 的管理员，不值得为它改代码 |
