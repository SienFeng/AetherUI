...
//
// **这里刻意与界面显示的口径不同。** locateWithIPDB 那条路径改用省市两级多数
// 票之后，同一个 IP 的画像键仍按源顺序算——画像键决定「是不是同一个网络」，
// 换口径会让升级瞬间一批 IP 的键跟着变，同一个网络在升级前后被算成两个，共享
// 检测的并存判定当场断档一次，而界面上不会有任何提示。显示口径变的是给人看的
// 字，判定口径变的是历史数据的可比性，两者该不该动是两个问题。
//
