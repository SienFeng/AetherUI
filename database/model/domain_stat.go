...
// 存在——查询条件是 Where(time >= 位点时间 AND (time > 位点时间 OR
// id > 位点id))（与更直白的 time > 位点时间 OR (time = 位点时间 AND
// id > 位点id) 逻辑完全等价，写成带前导 time >= 的形式是为了让 SQLite
// 能用上 Time 上的索引，见 DomainStatService.Aggregate 里查询语句上方的
// 注释），新行不管实际拿到的 id 是多少，只要写入时刻在位点之后就会被
// 读到，不需要任何自愈或回退逻辑；LastLogId 退化成只在同一毫秒内排序用
// 的次要字段。**这不是"同一毫秒内两行随机撞上"这种概率论意义上的小概率
// 事件**——真正的触发条件是确定的："位点尾部恰好停在某一毫秒" 与 "那一
// 毫秒之内、之后发生过释放高位 id 的删除" 同时成立：此时如果新数据又恰好
// 落进同一毫秒、复用了比位点 id 更小的号，会被 id > 位点id 这半句排除掉。
// 影响是有界的——只波及与位点尾部共享同一毫秒的那几条，方向是漏读（少算
// 几条，不会重复计数），且发生窗口极窄（一毫秒），实践中当作可以接受的
// 残留代价，但不是"概率可忽略"意义上的没有触发条件。
//
// 残留代价（刻意接受，不做补偿）：
//
//  1. 系统时钟回拨（NTP 步进、DST 秋季回拨每年一次）期间写入的日志，其
//     Time 小于当时的位点时间，会被这个查询条件永久跳过。后果是榜单少
//     一段数据，方向是漏数而不是重复计数，落在安全侧——这与本文件其它
//     地方「宁可漏读也不能重复计入」的取舍一致。
//  2. 更严重的一类：**系统时钟一度超前**（NTP 故障、误设系统时间、虚拟
//     机快照回滚后再前进）时写下的日志会把位点顶到未来某个时刻；时钟
//     校正回来之后，time >= 位点 从此永远不可能被真实数据满足，聚合
//     **永久停摆**——不是"漏一段"，是从那一刻起彻底不再更新，且停摆
//     幅度等于当时时钟前跳的量（可能是几小时到几天），没有边界。
//     AccessLogCleanupJob 的清理条件是 time < cutoff，永远删不到这些
//     "来自未来"的行，这个状态不会自愈。触发源不止直接改系统时间一种：
//     面板与 xray 是两个独立进程，`time.Local` 是进程内缓存，如果在两者
//     的重启窗口内改过系统时区，两个进程对同一时刻的本地时间解读可能
//     整体错位，最多相差 26 小时（两个时区偏移之差的极值）。
//     DomainStatService.Aggregate 对此只做侦测（位点时间超前当前系统
//     时间超过 24 小时即记一条 Warning）、不做自愈——见该函数注释，
//     自愈在这条路上已经反复出错，宁可让管理员看到告警后手工处理。
//
